...
Target \texttt{nvfortran-openacc-force-oracle} is a model-independent,
development-only GPU build. For supported Euler, RK2, RK4, and fixed-topology
Platen paths, with or without evaporation, it downloads the current force
evaluation state, evaluates the complete trusted host equations, uploads only
their derivatives, and leaves integration updates and dynamic topology on the
GPU. It enables the single
\texttt{JETSPIN\_DEV\_HOST\_FORCE\_ORACLE} macro and covers non-evaporative
Platen, Maxwell evaporative Platen, and both deterministic evaporation
rheologies.

Target \texttt{nvfortran-openacc-coulomb-oracle} is the narrower diagnostic.
It enables only \texttt{JETSPIN\_DEV\_HOST\_COULOMB\_ORACLE}, transfers the
state required by the direct Coulomb sum, evaluates that sum on the host in
its established order, and uploads the force array. Its semantics are the same
with or without evaporation. Every other force term and the integration
updates remain on the GPU. The two targets are deliberately distinct, and
their per-stage transfers make both unsuitable for production or performance
measurements.

Target \texttt{nvfortran-openacc-host-akima} enables the development-only
\texttt{JETSPIN\_DEV\_HOST\_AKIMA} macro. It retains the historical host
coefficient and interpolation routines while leaving the surrounding OpenACC
refinement lifecycle enabled. Target
\texttt{nvfortran-openacc-compare-akima} enables
\texttt{JETSPIN\_COMPARE\_AKIMA}; it computes the trusted host result followed
by the device result and reports maximum coefficient and interpolated-value
differences for every field. Neither target is intended for performance
measurements.
...
nvfortran-openacc-force-oracle & compile supported Euler, RK2, RK4, or Platen
paths with the complete development-only host-force oracle.\tabularnewline
nvfortran-openacc-coulomb-oracle & compile supported Euler, RK2, RK4, or Platen
paths with only the direct Coulomb sum evaluated on the host.\tabularnewline
nvfortran-openacc-host-akima & retain the historical host Akima routines as a
development-only refinement oracle.\tabularnewline
nvfortran-openacc-compare-akima & compare host and device Akima coefficients
and interpolated values at every accepted refinement.\tabularnewline
