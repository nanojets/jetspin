...
Target \texttt{nvfortran-openacc-kv-host-forces} provides the corresponding
development-only diagnostic for Kelvin--Voigt evaporation. Every force stage
is evaluated by the trusted host routines and only its derivatives are copied
to the device; Runge--Kutta state updates remain on the GPU. The deliberate
per-stage transfers likewise make this target unsuitable for production or
...
nvfortran-openacc-kv-host-forces & compile the Kelvin--Voigt GPU path with
development-only host force evaluation and per-stage transfers.\tabularnewline
