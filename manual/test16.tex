...
the complete trusted Maxwell force stage on the host and copies its
derivatives back while state updates remain on the GPU.  This oracle reproduces
the Euler/RK2 CPU pre-event geometry and aggregate topology, but its transfers
make it unsuitable for production or performance runs.  The input is stored
in \texttt{examples/input-16/input.dat}.
...
build used by the script does not enable either host-force diagnostic macro.
