...
preserves this established semantics. The rheology-independent
\texttt{nvfortran-openacc-force-oracle} target evaluates every force stage
through the trusted host equations and uploads its derivatives. The narrower
\texttt{nvfortran-openacc-coulomb-oracle} target moves only the direct
Coulomb sum to the host. Both leave Runge--Kutta updates on the device and are
useful for numerical isolation, but unsuitable for performance measurements.
The input is stored in \texttt{examples/input-17/input.dat}.
